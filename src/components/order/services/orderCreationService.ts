...
export async function createOrder(params: CreateOrderParams) {
  ...
  try {
    ...
    if (params.orderType === "single") {
      return await createSingleOrderInternal(params, orderTotals, paymentSettings);
    } else {
      return await createSplitOrderInternal(params, orderTotals, paymentSettings);
    }
  } catch (error: any) {
...
// Keep the internal functions for backward compatibility with the main createOrder function
async function createSingleOrderInternal(params: CreateOrderParams, orderTotals: any, paymentSettings: any) {
  try {
    // Generate unique order number
    const orderNumber = `ORD-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
...
async function createSplitOrderInternal(params: CreateOrderParams, orderTotals: any, paymentSettings: any) {
  try {
    const masterOrderNumber = `SPL-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    const orders = [];
