...
export async function createSplitOrder(params: CreateSplitOrderParams) {
  try {
    ...
    for (let i = 0; i < params.splits.length; i++) {
      ...
      const splitCustomerAddress = isPickup ? pickupAddress : (split.sameAsBilling ? params.customer.full_address : split.deliveryAddress);
      const splitDeliveryAddress = isPickup ? pickupAddress : (split.sameAsBilling ? params.customer.full_address : split.deliveryAddress);
