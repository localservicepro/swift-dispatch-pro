...
import { Search, Package, Plus, Minus, Star, Clock } from "lucide-react";
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 max-h-[70vh] overflow-y-auto">
            {products.map((product) => {
              ...
              return (
                <div key={product.id} className="border rounded-lg p-4 relative hover:shadow-md transition-shadow">
                  {hasSpecial && (
                    ...
                    {cartQuantity > 0 ? (
                      <div className="flex items-center gap-2">
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => updateQuantity(product.id, cartQuantity - 0.25)}
                        >
                          <Minus className="w-3 h-3" />
                        </Button>
                        <span className="font-medium">{formatQuantity(cartQuantity)}</span>
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => updateQuantity(product.id, cartQuantity + 0.25)}
