...
-- Change delivery_time column from time to text to store formatted time
ALTER TABLE public.order_sms_webhooks 
ALTER COLUMN delivery_time TYPE text USING 
  CASE 
    WHEN delivery_time IS NOT NULL THEN TO_CHAR(delivery_time, 'HH12:MI AM')
    ELSE NULL
  END;
...
-- Backfill existing records with company name
UPDATE public.order_sms_webhooks osw
SET company_name = COALESCE(c.company_name, c.business_name)
FROM orders o
