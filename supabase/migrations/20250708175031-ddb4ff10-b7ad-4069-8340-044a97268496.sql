...
CREATE POLICY IF NOT EXISTS "Admins can manage WooCommerce sync settings" ON public.woocommerce_sync_settings
  FOR ALL USING (is_current_user_admin())
  WITH CHECK (is_current_user_admin());

-- Create RLS policies for woocommerce_sync_logs
CREATE POLICY IF NOT EXISTS "Admins can manage WooCommerce sync logs" ON public.woocommerce_sync_logs
  FOR ALL USING (is_current_user_admin())
  WITH CHECK (is_current_user_admin());

-- Create RLS policies for woocommerce_product_mapping
CREATE POLICY IF NOT EXISTS "Admins can manage WooCommerce product mapping" ON public.woocommerce_product_mapping
  FOR ALL USING (is_current_user_admin())
  WITH CHECK (is_current_user_admin());

-- Create RLS policies for woocommerce_category_mapping
CREATE POLICY IF NOT EXISTS "Admins can manage WooCommerce category mapping" ON public.woocommerce_category_mapping
  FOR ALL USING (is_current_user_admin())
  WITH CHECK (is_current_user_admin());
