...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Create a more structured response with better download handling
    const receiptData = {
      html: receiptHtml,
      invoiceNumber: invoice.invoice_number,
      orderNumber: invoice.orders.order_number,
    ...
    // Create a blob-compatible data URL
    const htmlBlob = new Blob([receiptHtml], { type: 'text/html' });
    const base64Html = btoa(receiptHtml);
    ...
    return new Response(
      JSON.stringify({ 
        success: true,
        receiptData,
        downloadUrl: `data:text/html;charset=utf-8;base64,${base64Html}`,
